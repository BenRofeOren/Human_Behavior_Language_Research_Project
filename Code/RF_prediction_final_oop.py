...
def make_report(config):
    ...
    results_directory = f'reports/pred_{label_column}_per_{group_by_feature}' \
                        f'_with{smote_string}_SMOTE_' \
                        f'normalization_{normalization_method}' \
                        f'_with_{no_of_k_folds}_fold'
    if remove_session_f:
        results_directory += '_remove_SESS_1_4'
    ...
    if remove_session_f:
        df = df.drop(df[df['SESS'] == 1].index)
        df = df.drop(df[df['SESS'] == 4].index)
    ...
    for group_name, group_df in grouped_df:
        ...
        while len(X_train_copy.columns) > remove_each_time:
...
config = {
    ...
    'group_by_feature': 'COND',  # separate data according to this feature
